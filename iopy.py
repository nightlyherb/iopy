...
class Search:
    ...
    def __init__(
        ...
        *,
        ...
        self.engine = sa.create_engine("sqlite:///" + database_filename, echo=True)
...
class SearchQueueDeque:
    def __init__(self: SearchQueueDeque) -> None:
        self._queue: deque[SearchState] = deque([])

    def enqueue(self: SearchQueueDeque, *, state: SearchState, cost: int) -> None:
        self._queue.append(state)

    def dequeue(self: SearchQueueDeque) -> SearchState:
        return self._queue.popleft()

    def is_empty(self: SearchQueueDeque) -> bool:
        return not self._queue
...
if __name__ == "__main__":
    search = Search(
        period=4,
        database_filename="./iopy_b3s23_a_w8.db",
    )
